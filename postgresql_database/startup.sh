...
# NOTE: This script is used by the container to bring PostgreSQL up and make it
# reachable for dependent services.
#
# Key fix:
# - The orchestration for this project exposes PostgreSQL on PGPORT=5001 (see .env).
# - The previous version hardcoded DB_PORT=5000 which caused readiness checks to fail
#   and broke dependent service startup.
...
# Respect container-level env var PGPORT (project convention) and fall back sensibly.
DB_PORT="${PGPORT:-${POSTGRES_PORT:-5001}}"
